...
import { IHomeSupport } from '@/Models/Home';
...
const Supported = ({ supports }: Props) => {
  ...
  const onSubmitHandler = () => {
    imageForm.post(route('home.support.store'), {
  ...
  const onEditHandler = (id: number) => {
    ...
    editImageForm.post(route('home.support.update', id), {
      onProgress: () => setLoading(true),
      onSuccess: () => {
        setLoading(false);
        // window.location.reload()
  ...
  const onDeleteHandler = (id: number) => {
    ...
    imageForm.delete(route('home.support.destroy', id), {
      onProgress: () => setLoading(true),
      onSuccess: () => {
        setLoading(false);
        // window.location.reload()
      ...
      <div className="overflow-hidden font-roboto flex flex-col gap-4 mt-4 px-2">
        <div className="flex flex-col md:flex-row justify-between gap-4">
          ...
          <div className='flex flex-row justify-between gap-4'>
            ...
            {!enabled ?
              <button className='px-4 py-2 w-32 bg-green-500 rounded-lg font-bold text-white cursor-not-allowed' onClick={onSubmitHandler} disabled>Add Photo</button>
              :
              <button className='px-4 py-2 w-32 bg-green-500 rounded-lg font-bold text-white' onClick={onSubmitHandler} >Add Photo</button>
            }
          </div>
        </div>
